...
class FCN8(BaseModel):
    def __init__(self, num_classes, pretrained=True, freeze_bn=False, freeze_backbone=False, **_):
        ...
        self.up_output = nn.ConvTranspose2d(
            num_classes, num_classes, kernel_size=4, stride=2, bias=False
        )
        self.up_pool4_out = nn.ConvTranspose2d(
            num_classes, num_classes, kernel_size=4, stride=2, bias=False
        )
    ...
    def forward(self, x):
        ...
        add_out_pool4 = self.up_pool4_out(
            adjstd_pool4[:, :, 5 : (5 + up_output.size()[2]), 5 : (5 + up_output.size()[3])]
            + up_output
        )
        ...
        final_value = self.up_final(
            adjstd_pool3[
                :,
                :,
                9 : (9 + add_out_pool4.size()[2]),
                9 : (9 + add_out_pool4.size()[3]),
            ]
            + add_out_pool4
        )
    ...
    def get_backbone_params(self):
        return chain(
            self.pool3.parameters(),
            self.pool4.parameters(),
            self.pool5.parameters(),
