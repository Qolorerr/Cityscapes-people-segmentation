    ...
    def __init__(
        ...
        **kwargs
        ...
        if self._without_padding:
            ...
            # Here, we assume that floor(padding) = padding
            padding = (kernel_size - 1) * rate // 2
            self._conv_depthwise = torch.nn.Conv2d(
                in_channels,
        ...
        self._conv_pointwise = torch.nn.Conv2d(
            out_channels_depthwise, out_channels, kernel_size=1, stride=1, bias=False, **kwargs
        )
    ...
    def __init__(
        ...
        if activation_fn_in_separable_conv:
            activation_fn = (
                ...
                else torch.nn.ReLU(inplace=False)
            )
        else:
            if use_bounded_activation:
        ...
        if skip_connection_type == "conv":
            self._conv_skip_connection = torch.nn.Conv2d(
                in_channels, depth_list[-1], kernel_size=1, stride=stride
            )
...
class Xception(torch.nn.Module):
    ...
    def __init__(
        ...
        layers += [Conv2dSame(3, 32, 3, stride=2), Conv2dSame(32, 64, 3, stride=1)]
        ...
        if num_classes:
            layers += [
                torch.nn.Dropout2d(p=keep_prob, inplace=True),
                torch.nn.Conv2d(blocks[-1].args[-1]["depth_list"][-1], num_classes, 1),
            ]
