...
def sliding_predict(model, image, num_classes, flip=True):
    ...
    for row in range(num_rows):
        for col in range(num_cols):
            ...
            total_predictions[:, y_min:y_max, x_min:x_max] += (
                predictions.data.cpu().numpy().squeeze(0)
            )
...
def multi_scale_predict(model, image, scales, num_classes, device, flip=False):
    ...
    for scale in scales:
        scaled_img = ndimage.zoom(
            image, (1.0, 1.0, float(scale), float(scale)), order=1, prefilter=False
        )
...
def parse_arguments():
    ...
    parser.add_argument("-o", "--output", default="outputs", type=str, help="Output Path")
    parser.add_argument(
        "-e",
        "--extension",
        default="jpg",
